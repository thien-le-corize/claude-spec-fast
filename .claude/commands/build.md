...
- A plan exists in `.claude/specs/tasks.md`
- Understanding of task acceptance criteria

## CRITICAL RULES

1. **Read tasks from `.claude/specs/tasks.md`** (global) or `.claude/specs/<phase>/tasks.md` (child)
2. **Update task status in the same file** when done
3. **Never create a `tasks/` folder at project root**
4. **When starting a new phase**, create child spec folder `.claude/specs/<phase-name>/` with its own tasks.md
...
Update `.claude/specs/<project-name>/tasks.md`:
```markdown
...
- Working, tested code
- Updated `.claude/specs/tasks.md` with completed items (mark status → done)
- When a phase is done, update global task status and move to next phase
